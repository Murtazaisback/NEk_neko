...
export const Item = ({
    label,onClick,icon: Icon,id,documentIcon,active,expanded,isSeearch,level = 0,onExpand
}: ItemsProps)=>{
    const {user} = useUser();
    ...
    return(
        <div role="button" onClick={onClick} style={{ paddingLeft: level ? `${(level * 12) + 12}px` : "12px"}} className={cn("group min-h-[27px] text-sm py-1 pr-3 w-full hover:bg-primary/5 flex items-center text-muted-foreground font-medium",
        active && "bg-primary/5 text-primary"
            ...
            {isSeearch && (
                <kbd className="ml-auto pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border bg-muted pr-1.5 font-mono text-[10px] font-medium text-muted-foreground opacity-100">
